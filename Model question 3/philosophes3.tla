...
request_both_forks(i) ==
    /\ state[i] = Hungry  \* Transition to request both forks
    /\ forks_state[left(i)] = Unhold  \* Ensure left fork is unheld
    /\ forks_state[right(i)] = Unhold  \* Ensure right fork is unheld
    /\ forks_state' = [ forks_state EXCEPT ![left(i)] = i, ![right(i)] = i ]  \* Hold both forks
    /\ UNCHANGED state
...
Fairness ==
    \A i \in Philos :
        ...
        /\ WF_<<state, forks_state>>(ask(i))  \* Weak fairness for ask transition
        /\ SF_<<state, forks_state>>(request_both_forks(i))  \* Strong fairness for request both forks transition
